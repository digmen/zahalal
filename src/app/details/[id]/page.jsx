...
const getData = async (id) => {
    try {
        const res = await fetch(`${API_URL}cards/details/${id}/`, {
            cache: 'no-cache'
        });
            ...
            <main className='max-w-[960px] mx-auto my-0 max-lg:max-w-[720px]'>
                ...
                <article className='py-7 px-4'>
                    <section>
                        <div className='flex items-center justify-between'>
                            <h1 className="text-[24px] leading-[28px] tracking-[-0.12px] font-bold text-[#131105]">
                                <span className='max-sm:text-[18px] text-2xl font-bold'>{restaurant.title}</span>
                            </h1>
                            <ShareButton />
                        </div>
                        <span className='max-sm:text-[12px] text-sm text-[#656565]'>{restaurant.address}</span>
                    </section>
                    <section>
                        <div className='inline-flex items-center gap-2 '>
                            <span className='max-sm:text-[14px] text-[16px] font-normal text-black leading-6'>
                                {restaurant.average_rating || 'Нет рейтинга'}
                            </span>
                            <Image src='/images/star.svg' alt='star' width={16} height={16} />
                        </div>
                    </section>
                    <section className='w-full py-3'>
                        <div className='flex items-center  gap-4 w-full overflow-x-scroll overflow-y-hidden '>
                            <button className='group cursor-pointer flex flex-1 items-center justify-center backdrop-blur-sm h-10 hover:bg-[#131105] hover:text-white text-[18px] text-black font-normal bg-[#F3F0F3] p-2 px-4 rounded-lg transition-all duration-300 ease-in-out'>
                                <Link href={`tel:${restaurant.phone_number}`}
                                    className='max-sm:text-[14px] inline-flex gap-1.5 items-center justify-center'
                                >
                    ...
                    <section className='py-4'>
                        ...
                        <p className='max-sm:text-[14px] text-[#656565] text-[16px] font-normal leading-6'>
                            {restaurant.description}
                        </p>
                    </section>
                    <ModalViewAllPhoto restaurantPhoto={restaurant.photos} />
