...
export default async function Page({ searchParams }) {
  ...
  const searchCity = resolvedSearchParams?.city;
  ...
  // 3. Получаем карточки
  // === ВАЖНО: если есть параметры поиска по городу, делаем поиск по searcher API ===
  let cardsData;

  if (searchQuery || searchCity) {
    // searchQuery может быть пустой строкой!
    let url = `${API_URL}cards/searcher?`;
    if (searchCity) url += `city=${encodeURIComponent(searchCity)}&`;
    if (searchQuery) url += `search=${encodeURIComponent(searchQuery)}&`;
    url = url.replace(/[&?]+$/, ''); // убираем лишние & или ?

    const cardsRes = await fetch(url, { cache: 'no-cache' });
    cardsData = await cardsRes.json();
  } else {
    // Обычный список по категории
      ...
      <article className='mt-8 gap-9 flex items-start justify-between lg:gap-4 max-lg:px-6 max-sm:px-4'>
        ...
        <section className='flex flex-col gap-6 w-full my-[50px]'>
          ...
          <section className='flex flex-col gap-6 w-full'>
            ...
            <Card cards={Array.isArray(cardsData.results) ? cardsData.results : cardsData} />
          </section>
        </section>
      </article>
    </main>
